...
namespace MicroEngineer.Windows
{
    public class ManeuverWindow : EntryWindow
    {
        ...
        public int DeleteNodes()
        {
            ...
            foreach (ManeuverNodeData node in nodes)
            {
                if (!nodesToDelete.Contains(node) &&
                    (!nodeToDelete.IsOnManeuverTrajectory || nodeToDelete.Time < node.Time))
                    nodesToDelete.Add(node);
            }

            GameManager.Instance.Game.SpaceSimulation.Maneuvers.RemoveNodesFromVessel(Utility.ActiveVessel.GlobalId,
                nodesToDelete);
            SelectedNodeIndex = 0;
        ...
        {
            ManeuverPlanComponent activeVesselPlan =
                Utility.ActiveVessel.SimulationObject.FindComponent<ManeuverPlanComponent>();
