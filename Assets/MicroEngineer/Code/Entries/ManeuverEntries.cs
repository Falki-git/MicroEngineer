...
namespace MicroEngineer.Entries
{
    ...
    public class DeltaVRequired : ManeuverEntry
    {
        ...
        {
            List<ManeuverNodeData> nodes = Utility.ActiveVessel?.SimulationObject
                ?.FindComponent<ManeuverPlanComponent>()?.GetNodes();

            EntryValue = (nodes == null || nodes.Count == 0)
                ? null
                :
                (nodes.Count == 1)
                    ?
                    (Utility.ActiveVessel.Orbiter.ManeuverPlanSolver.GetVelocityAfterFirstManeuver(out double ut)
                        .vector - Utility.ActiveVessel.Orbit.GetOrbitalVelocityAtUTZup(ut)).magnitude
                    :
                    (nodes.Count >= base.SelectedNodeIndex + 1)
                        ? nodes[base.SelectedNodeIndex].BurnRequiredDV
                        :
                        null;
        }
