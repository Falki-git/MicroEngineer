...
namespace MicroEngineer.Entries
{
    ...
    public class StageISPActual : VesselEntry
    {
        public StageISPActual()
        {
            Name = "ISP";
            Description =
                "Specific impulse (ISP) is a measure of how efficiently a reaction mass engine creates thrust.";
            Category = MicroEntryCategory.Vessel;
            IsDefault = false;
            BaseUnit = "s";
    ...
    public class NonStageableResourcesEntry : VesselEntry
    {
        ...
        public override void RefreshData()
        {
            EntryValue = ParseNonstageableResources(Utility.ActiveVessel.SimulationObject?.PartOwner?.ContainerGroup
                .GetAllResourcesContainedData().ToList());
        }
        ...
        private List<NonStageableResource> ParseNonstageableResources(List<ContainedResourceData> resources)
        {
            ...
            foreach (var resource in resources)
            {
                var resourceDefinition =
                    GameManager.Instance.Game.ResourceDefinitionDatabase.GetDefinitionData(resource.ResourceID);
