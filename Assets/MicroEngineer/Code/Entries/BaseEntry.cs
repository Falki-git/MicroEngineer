...
namespace MicroEngineer.Entries
{
    ...
    public class BaseEntry
    {
        ...
        [JsonProperty] public MicroEntryCategory Category;
        public EntryType EntryType;
        public bool IsDefault;
        [JsonProperty] public bool HideWhenNoData;
        [JsonProperty] public string MiliUnit;
        [JsonProperty] public string BaseUnit;
        [JsonProperty] public string KiloUnit;
        [JsonProperty] public string MegaUnit;
        [JsonProperty] public string GigaUnit;
        [JsonProperty] public AltUnit AltUnit;
        [JsonProperty] public byte NumberOfDecimalDigits;
        [JsonProperty("Formatting")] private string _formatting;
                ...
                if (value != entryValue)
                {
                    ...
                    switch (EntryType)
                    {
                        ...
                        case EntryType.Time:
                            ...
                            OnEntryTimeValueChanged?.Invoke(time.Years, time.Days, time.Hours, time.Minutes,
                                time.Seconds);
                            break;
        ...
        public virtual string ValueDisplay
        {
            get
            {
                ...
                {
                    return !string.IsNullOrEmpty(Formatting)
                        ? String.Format(Formatting, d * AltUnit.Factor)
                        : (d * AltUnit.Factor).ToString();
                }

                if (Math.Abs(d) < 1) // mili
                {
                    return !String.IsNullOrEmpty(this.MiliUnit)
                        ? String.Format(Formatting, d * 1000)
                ...
                {
                    return !String.IsNullOrEmpty(this.KiloUnit)
                        ? String.Format(Formatting, d / 1000)
                        : String.Format(Formatting, d);
