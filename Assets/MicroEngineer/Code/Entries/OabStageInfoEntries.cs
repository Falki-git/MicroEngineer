...
namespace MicroEngineer.Entries
{
    ...
    public class Torque : OabStageInfoEntry
    {
        public Torque()
        {
            Name = "Torque";
            Description =
                "Thrust torque that is generated by not having Thrust Vector and Center of Mass aligned. Turn on the Center of Thrust and Center of Mass VAB indicators to get an accurate value.";
            Category = MicroEntryCategory.OAB;
            IsDefault = true;
            BaseUnit = null;
            Formatting = null;
        ...
        public override void RefreshData()
        {
            Vector3d com = GameManager.Instance?.Game?.OAB?.Current?.Stats?.MainAssembly?.CenterOfProperties?.CoM ??
                           Vector3d.zero;
            Vector3d cot = GameManager.Instance?.Game?.OAB?.Current?.Stats?.MainAssembly?.CenterOfProperties?.CoT ??
                           Vector3d.zero;

            if (com == Vector3d.zero || cot == Vector3d.zero)
                return;

            List<DeltaVEngineInfo> engines = GameManager.Instance?.Game?.OAB?.Current?.Stats?.MainAssembly?.VesselDeltaV
                ?.EngineInfo;
        ...
        public override string ValueDisplay
        {
            get
            {
                ...
                return Math.Abs((double)EntryValue) > double.Epsilon
                    ? $"{String.Format("{0:F2}", (double)EntryValue * 1000.0)}"
                    : $"{String.Format("{0:F0}", (double)EntryValue)}";
            }
        }
    }
            ...
            for (int i = 0; i < Utility.VesselDeltaVComponentOAB.StageInfo.Count; i++)
            {
                ...
                var stage = new DeltaVStageInfo_OAB()
                {
                    ...
                    SituationCelestialBody = retrieved.DeltaVSituation.CelestialBody == MicroCelestialBodies.HomeWorld
                        ? string.Empty
                        : retrieved.DeltaVSituation.CelestialBody,
                ...
                OnStageInfoOABChanged?.Invoke(((List<DeltaVStageInfo_OAB>)EntryValue)
                    .Where(s => s.DeltaVVac > 0.0001 || s.DeltaVASL > 0.0001).ToList());
            }
        }
    ...
    public class DeltaVStageInfo_OAB
    {
        ...
        private float GetThrustAtAltitude(double altitude, CelestialBodyComponent cel) => this.DeltaVStageInfo
            .EnginesActiveInStage
            ?.Select(e => e.Engine.MaxThrustOutputAtm(atmPressure: cel.GetPressure(altitude) / 101.325))?.Sum() ?? 0;
        ...
        private double GetISPAtAltitude(double altitude, CelestialBodyComponent cel)
        {
            ...
            foreach (DeltaVEngineInfo engInfo in this.DeltaVStageInfo.EnginesActiveInStage)
                sum += engInfo.Engine.MaxThrustOutputAtm(atmPressure: cel.GetPressure(altitude) / 101.325) /
                       engInfo.Engine.currentEngineModeData.atmosphereCurve.Evaluate((float)cel.GetPressure(altitude) /
                           101.325f);
            return GetThrustAtAltitude(altitude, cel) / sum;
        }

        private double GetDeltaVelAlt(double altitude, CelestialBodyComponent cel) => GetISPAtAltitude(altitude, cel) *
            9.80665 * Math.Log(this.DeltaVStageInfo.StartMass / this.DeltaVStageInfo.EndMass);

        private double GetTWRAtAltitude(double altitude, CelestialBodyComponent cel) => this.DeltaVStageInfo.TWRVac *
            (GetThrustAtAltitude(altitude, cel) / this.DeltaVStageInfo.ThrustVac);
        ...
        public void Recalculate_SLT() => this.TWRASL =
            (float)(this.GetTWRAtSeaLevel(this.CelestialBody.CelestialBodyComponent) * this.CelestialBody.TwrFactor);

        public void Recalculate_DeltaVSeaLevel() =>
            this.DeltaVASL = this.GetDeltaVelAtSeaLevel(this.CelestialBody.CelestialBodyComponent);
