...
namespace MicroEngineer.Utilities
{
    ...
    public static class TransferInfo
    {
        ...
        private static bool CalculateParameters()
        {
            ...
            try
            {
                from = (Utility.ActiveVessel.Orbit.referenceBody, Utility.ActiveVessel.Orbit.Position.localPosition,
                    Utility.ActiveVessel.Orbit);
                to = (Utility.ActiveVessel.TargetObject.Orbit.referenceBody,
                    Utility.ActiveVessel.TargetObject.Orbit.Position.localPosition,
                    Utility.ActiveVessel.TargetObject.Orbit);
            }
            catch (Exception _)
            {
        ...
        public static double? GetPhaseAngle()
        {
            ...
            {
                double phase = Vector3d.SignedAngle((Vector3d)_toLocalPosition, (Vector3d)_fromLocalPosition,
                    Vector3d.up);
        ...
        public static double? GetTransferAngle()
        {
            if (CalculateParameters())
            {
                ...
                double time = Mathf.PI * Mathf.Sqrt((float)((ellipseA) * (ellipseA) * (ellipseA)) /
                                                    ((float)_toOrbit.referenceBody.Mass * 6.67e-11f));
