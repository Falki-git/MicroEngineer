...
namespace MicroEngineer.Utilities
{
    public static class Utility
    {
        ...
        public static string LayoutPath = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location),
            "MicroLayout.json");
        ...
        public static void RefreshActiveVesselAndCurrentManeuver()
        {
            ...
            CurrentManeuver = ActiveVessel != null
                ? GameManager.Instance?.Game?.SpaceSimulation.Maneuvers.GetNodesForVessel(ActiveVessel.GlobalId)
                    .FirstOrDefault()
                : null;
        }
        ...
        public static void LoadLayout(List<BaseWindow> windows)
        {
            try
            {
                List<BaseWindow> deserializedWindows =
                    JsonConvert.DeserializeObject<List<BaseWindow>>(File.ReadAllText(LayoutPath));
                ...
                {
                    Logger.LogInfo(
                        "Loaded layout version is older than the current supported version. Layout will be reset.");
                    return;
                }
            ...
            {
                Logger.LogWarning(
                    $"MicroLayout.json file was not found at the expected location during LoadLayout. This is normal if this mod was just installed. Window states and positions will keep their default values.");
        ...
        public static bool AreRectsNear(Rect rect1, Rect rect2)
        {
            ...
            return (distanceX < rect1.width / 2 + rect2.width / 2 + 50f &&
                    distanceY < rect1.height / 2 + rect2.height / 2 + 50f);
        }
        ...
        public static void CenterWindow(GeometryChangedEvent evt, VisualElement element, EventCallback<GeometryChangedEvent> callback)
        {
            if (evt.newRect.width == 0 || evt.newRect.height == 0)
                return;

            element.transform.position = new Vector2((ReferenceResolution.Width - evt.newRect.width) / 2,
                (ReferenceResolution.Height - evt.newRect.height) / 2);

            element.UnregisterCallback<GeometryChangedEvent>(callback);
        }

        [CanBeNull]
        public static Dictionary<string, CelestialBodyScienceRegionsData> GetScienceRegions()
        {
            var scienceRegionsProvider =
